...
class TestANEStructs:
  ...
  def test_kernel_size_defaults(self):
    from mcgyver.header_index.os_frameworks.mac.ane import ANECKernelSize
    ks = ANECKernelSize()
    assert ks.depth == 1
    assert ks.height == 1
    assert ks.width == 1
  ...
  def test_step_to_bytes(self):
    from mcgyver.header_index.os_frameworks.mac.ane import ANECStep
  ...
  def test_padding_to_bytes(self):
    from mcgyver.header_index.os_frameworks.mac.ane import ANECPadding
  ...
  def test_tensor_dims_defaults(self):
    from mcgyver.header_index.os_frameworks.mac.ane import ANECTensorDims
    dims = ANECTensorDims()
    assert dims.n == 1
    assert dims.c == 1
    assert dims.h == 1
...
class TestANECompiler:
  ...
  @pytest.fixture
  def ane(self):
    from mcgyver.header_index.os_frameworks.mac.ane import ANECompiler
    return ANECompiler()
...
class TestANEHelpers:
  ...
  def test_get_ane_info(self):
    ...
    from mcgyver.header_index.os_frameworks.mac.ane import get_ane_info
    info = get_ane_info()
  ...
  def test_struct_sizes_constant(self):
    ...
    from mcgyver.header_index.os_frameworks.mac.ane import STRUCT_SIZES
  ...
  def test_probe_all_layer_desc_sizes(self):
    ...
    from mcgyver.header_index.os_frameworks.mac.ane import probe_all_layer_desc_sizes
...
class TestANESample:
  ...
  def test_simple_cnn(self):
    ...
    from mcgyver.header_index.os_frameworks.mac.ane import build_simple_cnn
  ...
  def test_transformer_attention(self):
    ...
    from mcgyver.header_index.os_frameworks.mac.ane import build_transformer_attention
...
class TestEspressoDiscovery:
  ...
  def test_list_espresso_classes(self):
    from mcgyver.header_index.os_frameworks.mac.ane import list_espresso_classes
  ...
  def test_list_class_methods(self):
    from mcgyver.header_index.os_frameworks.mac.ane import list_class_methods
...
class TestEspressoFormat:
  ...
  def test_find_system_models(self):
    from mcgyver.header_index.os_frameworks.mac.ane import find_system_models
  ...
  def test_is_json_format(self):
    from mcgyver.header_index.os_frameworks.mac.ane import find_system_models, is_json_format
  ...
  def test_parse_json_model(self):
    from mcgyver.header_index.os_frameworks.mac.ane import EspressoNet, find_system_models, is_json_format
  ...
  def test_espresso_layer_parsing(self):
    from mcgyver.header_index.os_frameworks.mac.ane import EspressoLayer
...
class TestPBZE:
  ...
  def test_is_pbze(self):
    from mcgyver.header_index.os_frameworks.mac.ane import is_pbze
  ...
  def test_find_pbze_file(self):
    from mcgyver.header_index.os_frameworks.mac.ane import is_pbze_file, find_system_models
  ...
  def test_decode_pbze(self):
    from mcgyver.header_index.os_frameworks.mac.ane import is_pbze_file, load_pbze_json, find_system_models
  ...
  def test_get_pbze_stats(self):
    from mcgyver.header_index.os_frameworks.mac.ane import is_pbze_file, get_pbze_stats, find_system_models
  ...
  def test_decode_espresso_net_json(self):
    from mcgyver.header_index.os_frameworks.mac.ane import decode_espresso_net, is_pbze_file, find_system_models
...
class TestANEXPC:
  ...
  def test_get_ane_xpc_info(self):
    from mcgyver.header_index.os_frameworks.mac.ane import get_ane_xpc_info
  ...
  def test_list_daemon_connection_methods(self):
    from mcgyver.header_index.os_frameworks.mac.ane import list_daemon_connection_methods
  ...
  def test_list_client_methods(self):
    from mcgyver.header_index.os_frameworks.mac.ane import list_client_methods
  ...
  def test_create_ane_client(self):
    from mcgyver.header_index.os_frameworks.mac.ane import create_ane_client
  ...
  def test_xpc_operations_structure(self):
    from mcgyver.header_index.os_frameworks.mac.ane import XPC_OPERATIONS, REQUIRED_ENTITLEMENTS
...
class TestAPITree:
  """Test API tree knowledge base."""
  
  def test_build_ane_api_tree(self):
    from mcgyver.header_index.os_frameworks.mac.ane import build_ane_api_tree
    
    tree = build_ane_api_tree()
    
    # Should have APIs
    assert len(tree.apis) >= 20
    
    # Should have examples
    assert len(tree.examples) >= 1
    
    # Should have capabilities
    assert len(tree.capabilities) >= 3
  ...
  def test_api_search(self):
    ...
    # Search for conv
    results = tree.search("conv")
    assert len(results) >= 2
    
    # Check result names
    names = [r.name for r in results]
    assert "ANECConvLayerDesc" in names
  ...
  def test_capability_lookup(self):
    ...
    # Get attention APIs
    apis = tree.get_apis_for_capability("attention")
    assert len(apis) >= 2
    
    # Should include SDPA
    names = [a.name for a in apis]
    assert "ANECSDPALayerDesc" in names
  ...
  def test_llm_context_generation(self):
    ...
    context = tree.to_llm_context("EspressoNetwork")
    
    # Should include key information
    assert "EspressoNetwork" in context
    assert "Espresso" in context
    assert "initWithJSFile" in context
    assert "Evidence" in context
  
  def test_evidence_confidence(self):
    from mcgyver.header_index.api_tree import APISignature, Evidence, EvidenceType
    ...
    # Add evidence and check confidence updates
    api.add_evidence(Evidence(
      type=EvidenceType.DYLD_EXPORT,
      description="Found in exports",
      confidence=0.5,
    ))
    
    assert api.confidence == 0.5
    
    # Add more evidence - confidence should increase
    api.add_evidence(Evidence(
      type=EvidenceType.RUNTIME_PROBE,
      description="Called successfully",
      confidence=0.8,
    ))
    
    # Compound confidence: 1 - (1-0.5)*(1-0.8) = 1 - 0.5*0.2 = 0.9
